...
public class FolderServiceFunc implements FolderService{
    ...
    public FolderResponse createFolder(CreateFolderReq req) {
        ...
        User owner = uRepository.findById(req.ownerId()).orElseThrow(() -> new NotFoundException("Owner not found", null));

        Folder parent = null;
        if (req.parentId() != null) {
            parent = fRepository.findById(req.parentId()).orElseThrow(() -> new NotFoundException("Parent not found", null));
        }
    ...
    public FolderResponse getFolder(GetFolderReq req) {
        ...
        Folder folder = fRepository.findByIdAndOwner_Id(req.folderId(), req.ownerId()).orElseThrow(() -> new NotFoundException("Folder does not exist", null));
